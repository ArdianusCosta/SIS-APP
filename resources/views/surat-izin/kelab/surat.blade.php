...
<head>
    ...
    <title>SIS-APP | Surat Izin Ke Lab</title>
...
<body class="bg-gray-100 font-sans leading-relaxed">
    <div class="button-bar fixed top-0 left-0 w-full bg-white shadow-md z-50">
        <div class="container mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <div class="text-gray-800 font-semibold text-lg">
                Total Surat Ke Lab yang Dibuat Hari Ini: {{ $suratHariIni->count() }}
            </div>
    ...
    @if($suratSebelumnya)
        <a href="{{ route('surat-kelab', ['id' => $suratSebelumnya->id]) }}"
    ...
    @if($suratBerikutnya)
        <a href="{{ route('surat-kelab', ['id' => $suratBerikutnya->id]) }}"
    ...
    <div class="surat-container container mx-auto mt-24 max-w-2xl bg-white p-8 rounded-lg shadow-lg">
        ...
        <h3 class="text-2xl font-bold text-center mb-4">SURAT IZIN KE LAB</h3>

        @php
            $sortedSuratHariIni = $suratHariIni->sortBy('created_at')->values();
            $nomor = $sortedSuratHariIni->search(function ($item) use ($keLab) {
        ...
        <p class="mb-4">{{ $keLab->kepada_yth }}</p>
        <p class="mb-4">Dengan hormat,</p>

        <p class="mb-4">Nama siswa yang mengajukan izin ke laboratorium adalah:</p>
        <table class="w-full mb-6">
            ...
            <tr>
                ...
                <td>: {{ $keLab->kelas->kelas }} - {{ $keLab->kelas->jurusan }}</td>
            </tr>
            <tr>
                <td class="py-2">Tanggal</td>
                <td>: {{ \Carbon\Carbon::parse($keLab->tanggal_izin)->translatedFormat('d F Y') }} -  {{ \Carbon\Carbon::parse($keLab->tanggal_selesai)->translatedFormat('d F Y') }}</td>
            </tr>
            <tr>
                <td class="py-2">Jam</td>
                <td>: {{ $keLab->jam_ke }} - {{$keLab->sampai_jam}}</td>
            </tr>
        </table>

        <p class="mb-2">Dengan alasan:</p>
        <p class="ml-6 mb-8">{{ $keLab->pesan_keluar_kelas }}</p>
        ...
        <div class="flex justify-between mt-12">
            ...
            <div class="w-5/12 text-center">
                ...
                <p>{{ $keLab->nama_penanggung_jawab }}</p>
            </div>
        </div>
    </div>
</body>
